...
public class SocialNetworkImpl implements SocialNetwork {
    ...
    public Set<UserProfile> getReachedUsers(Post post) {
        ...
        for (UserProfile user : registeredUsers) {
            ...
            if (hasCommonInterest && isWithinNetwork(author, user, new HashSet<>())) {
    ...
    private boolean isWithinNetwork(UserProfile origin, UserProfile target, Set<UserProfile> visited) {
        if (origin.isFriend(target)) {
            return true;
        }
        visited.add(origin);
        for (UserProfile friend : origin.getFriends()) {
            if (!visited.contains(friend) && isWithinNetwork(friend, target, visited)) {
                return true;
            }
        }
        return false;
